...
public class VerticalFormulaActivity extends Activity implements TextToSpeech.OnInitListener {
	...
	public void onInit(int status) {
		if (status == TextToSpeech.SUCCESS) {
			Toast.makeText(VerticalFormulaActivity.this, "成功输出语音", Toast.LENGTH_SHORT).show();
			// 设置使用美式英语朗读
			...
			if (result != TextToSpeech.LANG_COUNTRY_AVAILABLE
				...
				Toast.makeText(VerticalFormulaActivity.this, "TTS暂时不支持这种语言的朗读",
...
//					setData();
	...
	public void setData() {
		...
		while (mzimulist.size()<=0) {
			CommonVoid.makeshushitianci(mzimulist);
		}
		...
		vertical_imageView.setImageBitmap(bitmap);
